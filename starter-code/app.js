...
function changeSlideshow(current){
    ...
    const nextSlideshow = document.getElementById(`slideshow-${parseInt(current)+1}`);
    const previousSlideshow = document.getElementById(`slideshow-${parseInt(current)-1}`);

    if (currentSlideshow) {
        currentSlideshow.classList.remove("hidden");
        footerName.textContent = currentSlideshow.querySelector('h3').textContent;
        footerArtist.textContent = currentSlideshow.querySelector('.info-container-name > p').textContent;
